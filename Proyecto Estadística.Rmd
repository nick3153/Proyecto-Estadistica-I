...
Realizada la limpieza de datos, debemos mencionar que esta fue realizada a cad uno de los 48 meses con los que trabajamos, por lo tanto, mostramos una visra general de uno de los mese (que se repite en los demás meses)
...
costo_mercancia <- function(tablas_año, año){
  costos_dolares <- numeric()
  
  for (tabla in tablas_año) {
    valores <- get(tabla)$PBK * get(tabla)$VACID
    
    costos_dolares <- c(costos_dolares, valores)
  }
  
  
  # Calcular estadísticas descriptivas extendidas
  df_resumen <- data.frame(
    Año = año,
    Min = min(costos_dolares, na.rm = TRUE),
    Mean = mean(costos_dolares, na.rm = TRUE),
    Max = max(costos_dolares, na.rm = TRUE),
    Varianza = var(costos_dolares, na.rm = TRUE),
    Desviacion_Estandar = sd(costos_dolares, na.rm = TRUE),
    Sesgo = skewness(costos_dolares, na.rm = TRUE)
  )
...
- **5. Conclusiones**
Através de nuestro estudio logramos observar que efectivamente la pandemia comprometió las actividades de importación, siendo reflejado en una **disminución** de frecuencia, del precio y del costo del flete durante el 2019 que fue el atisbo del suceso, donde podemos llamar al acontecimiento como una recesión económica y después se refleja una alza exponencial dentro de 2021 y 2022 después de pandemia donde los items analizados se dispararon  
