...
Limpieza de datos: PBK, FLETES, VACID, VADUA
...
```{r}
#Gráfico general de 4 años Promedio PBK ---------------------------------------
...
# Imprimir resultados
print("Importación total 2019:", impo_total_2019, "\n")
print("Importación total 2020:", impo_total_2020, "\n")
print("Importación total 2021:", impo_total_2021, "\n")
print("Importación total 2022:", impo_total_2022, "\n")

```

```{r}
# Calcular promedios PBK mensuales para 2019 ---------------------------------------
# Crear un data frame con dos columnas: Mes y Promedio
promedios_2019 <- data.frame(
  Mes = nombre_tablas_2019,
  Promedio = sapply(nombre_tablas_2019, function(tabla) {
    # na.rm = TRUE ignora valores NA (no disponibles) en el cálculo
    mean(get(tabla)$PBK, na.rm = TRUE)
  })
)

# Crear gráfico de barras usando ggplot2
ggplot(promedios_2019, aes(x = Mes, y = Promedio)) +
  geom_col(fill = "#310541", color = "black", alpha = 0.7, position = position_dodge(width = 0.9)) +
  #geom_text(aes(label = comma(Promedio)), vjust= - 0.5, size = 4) +
  theme_minimal() +
  labs(title = "Promedio Precio Mercancía 2019", x = "Mes", y = "Promedio Precio Mercancía") +
  theme(
    axis.text.x = element_text(angle = 45, hjust = 1), plot.title = element_text(hjust = 0.5)
  )

print(promedios_2019)
```
